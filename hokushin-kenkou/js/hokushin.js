...
(function(){
  ...
  var VIEWS = {
    top:     { no:'01', label:'ホーム',            dark:false },
    reform:  { no:'02', label:'リフォーム',        dark:true  },
    newbuild:{ no:'03', label:'注文住宅',          dark:true  },
    prhub:   { no:'04', label:'ご依頼を検討中の方へ', dark:false },
    service: { no:'05', label:'業務案内',          dark:false },
    works:   { no:'06', label:'施工実績・お客様の声', dark:true  },
    price:   { no:'07', label:'料金表',            dark:false },
    staff:   { no:'08', label:'スタッフ紹介',      dark:false },
    faq:     { no:'09', label:'よくある質問',      dark:false },
    history: { no:'10', label:'北新建工のあゆみ',  dark:false },
    company: { no:'11', label:'会社概要',          dark:false },
    recruit: { no:'12', label:'採用情報',          dark:true  },
    contact: { no:'13', label:'お問い合わせ',      dark:false },
    blog:    { no:'14', label:'ブログ',            dark:false }
  };
  ...
  var WORKS=[
    {num:'01', title:'浴室のユニットバス化', meta:'水回りリフォーム ／ 上尾市 ／ 費用目安100万円前後・工期7日（※仮）', bg:_res('workImg0','assets/hs-beforeafter.jpg'),  thumb:_res('workImg0','assets/hs-beforeafter.jpg')},
    {num:'02', title:'フローリング全面張替え', meta:'フローリング ／ 桶川市 ／ 費用目安15万円前後・工期2日（※仮）', bg:_res('workImg1','assets/hs-room-closet.jpg'),  thumb:_res('workImg1','assets/hs-room-curtain.jpg')},
    {num:'03', title:'増改築工事（子供部屋増設）', meta:'増改築 ／ 伊奈町 ／ 費用目安280万円前後・工期約1ヶ月（※仮）', bg:_res('workImg2','assets/hs-corridor-wood.jpg'), thumb:_res('workImg2','assets/hs-demolition.jpg')}
  ];
  ...
  function setHeaderTone(){
    ...
    hdr.classList.toggle('stuck', stuck);
    hdr.classList.toggle('on-dark', !stuck && !!meta.dark);
    hdr.classList.toggle('on-light', !stuck && !meta.dark);
  }
  function onProg(){
